...
export const speechRecognition: Rule = {
  ...
  human: {
    explainer:
      "annyang and react-speech-recognition both wrap the same underlying API, adding phrase matching or restart-on-silence logic around it. The API itself lets a page listen for speech and hands back a transcript with confidence scores. The constructor is the one part worth copying carefully: the unprefixed name only arrived in Chrome 139, and Safari has exposed it as webkitSpeechRecognition for years, so read both off the window before calling it.",
    snippet: `// Unprefixed is Chrome 139 and up. Safari has only the prefixed name,
...
if (SpeechRecognitionCtor) {
  ...
  agent: {
    ...
    unless: [
      "You want one constructor name that works everywhere. The unprefixed SpeechRecognition is Chrome and Edge 139 and up, Safari has only webkitSpeechRecognition, and Firefox ships neither, so the feature detection is yours to write.",
